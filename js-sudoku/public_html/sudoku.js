...
function start() {
    ...
    drawBox(currentPuzzle);
}

function clear() {
    currentPuzzle = solveSudoku(currentPuzzle);
    drawBox(currentPuzzle);
}

function drawBox(puzzle) {
    area.innerHTML = "";
    var contents = "";
    contents += '<table class="sudoku-table">';
    for (var i = 0; i < puzzle.length; i++) {
        contents += '<tr class="row">';
        for( var j = 0; j < puzzle[i].length; j++) {
            var value = puzzle[i][j] == 0 ? "" : puzzle[i][j];
            contents += 
            '<td class="box"><input class="number-box" type="text" value="' + 
            value + 
            '" /></td>';
        }
        contents += '</tr>';
    }
    area.innerHTML = contents;
}
